...
import { isNative, nativeService } from '../services/nativeService'; // Import Native Bridge
...
const GPayLogo = () => (
  <svg width="45" height="18" viewBox="0 0 45 18" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path d="M7.18 0C3.21 0 0 3.23 0 7.23C0 10.97 2.91 14.12 6.54 14.43V7.22H3.92V4.83H6.54V2.78C6.54 0.65 8.32 0 10.03 0H10.5V2.39H10.19C9.17 2.39 8.92 2.87 8.92 3.5V4.83H11.23L10.74 7.22H8.92V14.33C12.15 13.68 14.56 10.73 14.56 7.23C14.56 3.23 11.26 0 7.18 0Z" fill="white"/>
    <path d="M19.34 4.88C17.65 4.88 16.36 5.8 15.68 6.94L15.6 6.94L15.42 5.09H13.6V14.16H15.65V9.41C15.65 8.16 16.63 7.02 17.87 7.02C18.99 7.02 19.46 7.82 19.46 8.93V14.16H21.51V8.66C21.51 6.35 20.65 4.88 19.34 4.88Z" fill="white"/>
    <path d="M25.75 4.88C23.63 4.88 22.06 6.5 22.06 8.78C22.06 11.04 23.59 12.69 25.73 12.69C26.56 12.69 27.27 12.44 27.87 11.96L27.94 11.96L28.07 12.51H29.78V5.09H27.91V6.26L27.83 6.26C27.31 5.39 26.55 4.88 25.75 4.88ZM26.04 10.88C24.96 10.88 24.11 9.99 24.11 8.79C24.11 7.56 24.96 6.69 26.04 6.69C27.13 6.69 27.97 7.56 27.97 8.79C27.97 10.02 27.13 10.88 26.04 10.88Z" fill="white"/>
    <path d="M35.69 5.09L33.37 11.66L33.31 11.66L30.93 5.09H28.79L32.28 13.88L30.34 18H32.42L37.93 5.09H35.69Z" fill="white"/>
    <path d="M42.27 10.22C42.27 12.01 40.91 12.78 39.46 12.78C38.2 12.78 37.14 12.24 36.65 11.39L38.29 10.63C38.52 11.06 38.92 11.3 39.43 11.3C40.06 11.3 40.48 10.97 40.48 10.51C40.48 9.53 37.69 9.38 37.69 7.33C37.69 5.86 39.02 4.88 40.45 4.88C41.56 4.88 42.45 5.34 42.84 6.07L41.34 6.79C41.13 6.42 40.83 6.3 40.4 6.3C39.87 6.3 39.52 6.6 39.52 7.02C39.52 7.89 42.27 7.84 42.27 10.22Z" fill="white"/>
  </svg>
);

export const GooglePayButton = ({ onClick }: { onClick: () => void }) => (
  <button 
    onClick={onClick}
    className="w-full bg-black hover:bg-gray-900 text-white rounded-full h-12 flex items-center justify-center gap-3 transition-colors border border-slate-700 shadow-lg relative overflow-hidden group"
  >
    <div className="absolute inset-0 bg-white/10 translate-y-full group-hover:translate-y-0 transition-transform duration-300"></div>
    <span className="text-sm font-medium relative z-10">Pay with</span>
    <div className="relative z-10 pt-1"><GPayLogo /></div>
  </button>
);
...
export const GoogleAdUnit = ({ slotId }: { slotId?: string }) => {
  useEffect(() => {
    // Attempt to push the ad to Google's queue (only if web)
    try {
      if (!isNative && window.adsbygoogle) {
  ...
  if (isNative) return null; // Native uses Interstitials, handled in logic

  if (!slotId) {
    // Placeholder if no real Slot ID is provided yet
    return (
      <div className="w-full h-full bg-slate-800 flex items-center justify-center text-slate-500 text-xs border border-slate-700">
  ...
  return (
    <ins className="adsbygoogle"
         style={{ display: 'block' }}
         data-ad-client="ca-pub-YOUR_PUBLISHER_ID" // REPLACE THIS WITH YOUR REAL ID IN CODE
         data-ad-slot={slotId}
         data-ad-format="auto"
...
export const AdOverlay = ({ onComplete }: { onComplete: () => void }) => {
  ...
  useEffect(() => {
    // Native Logic: Try to show interstitial first
    if (isNative) {
      const showNativeAd = async () => {
         const shown = await nativeService.showInterstitial();
         if (shown) {
           onComplete(); // Skip web timer if native ad shows
         }
      };
      showNativeAd();
    }
  ...
  return (
    <div className="fixed inset-0 bg-black z-[70] flex flex-col items-center justify-center p-6">
      ...
      {/* Container for Real Ads (Web Only) */}
      {!isNative && (
        <div className="w-full max-w-md h-64 bg-white/5 rounded-xl overflow-hidden mb-6 flex items-center justify-center relative">
          <GoogleAdUnit slotId="" /> 
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
              <span className="bg-black/50 px-3 py-1 rounded text-xs text-white">Ad Space</span>
          </div>
        </div>
      )}
      ...
      <div className="text-center space-y-6 max-w-sm">
        ...
        <p className="text-slate-400 text-sm">
          Watch this message to generate your video for free.
        </p>
