  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 400) {
        ...
        // Specific check for Body validation error
        if (lowerErr.includes('body validation error')) {
           throw new Error("Configuration Error: The AI model rejected the request format. Please try a simpler prompt or standard settings.");
        }
