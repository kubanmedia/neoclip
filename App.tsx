...
import { nativeService, isNative } from './services/nativeService';
import { Button, InputArea, LoadingOverlay, FunLoadingOverlay, PremiumBadge, LockOverlay, PremiumBadge as WatermarkBadge, Watermark, Toggle, TextInput, Toast, ImageUploader, AdOverlay, LottieAnimation, IdeaOverlay, GooglePayButton } from './components/UIComponents';
...
// Use a Payment Link that supports Google Pay (Web Fallback)
const PRO_PAYMENT_LINK = "https://buy.stripe.com/test_..."; 
const GOOGLE_ADSENSE_ID = "ca-pub-XXXXXXXXXXXXXXXX"; 
...
const App = () => {
  ...
  // Load Google AdSense Script (Web only) & Init Native Ads
  useEffect(() => {
    if (isNative) {
      nativeService.initializeAds();
    } else {
  ...
  const handleUpgrade = async () => {
    // NATIVE BILLING PATH (Google Play)
    if (isNative) {
      setIsGenerating(true);
      setGenStatus('Connecting to Google Play Store...');
      setGenProgress(20);
      
      try {
        const success = await nativeService.purchasePremium();
        if (success) {
          setGenProgress(100);
          setUser(prev => ({ ...prev, isPremium: true, credits: 100 }));
          setShowPaywall(false);
          setToastMessage("Subscription Activated!");
          setToastType('success');
        } else {
          setToastMessage("Purchase Canceled");
          setToastType('error');
        }
      } catch (e: any) {
        setToastMessage("Billing Error: " + e.message);
        setToastType('error');
      } finally {
    ...
    // WEB FALLBACK PATH (Google Pay via Stripe)
    if (PRO_PAYMENT_LINK.includes('http')) {
       window.open(PRO_PAYMENT_LINK, '_blank');
       setToastMessage("Opening Secure Checkout...");
       return;
    }
  ...
  const handleRestore = async () => {
    if (isNative) {
      setToastMessage("Checking Google Play History...");
      const restored = await nativeService.restorePurchases();
      if (restored) {
        setUser(prev => ({ ...prev, isPremium: true }));
        setToastMessage("Purchases Restored");
        setToastType('success');
      } else {
        setToastMessage("No subscription found.");
        setToastType('error');
      }
    } else {
      setToastMessage("Web Restore not supported in demo.");
    }
  };
        ...
        <div className={`p-6 rounded-2xl border relative overflow-hidden transition-all ${user.isPremium ? 'bg-gradient-to-br from-purple-900/40 to-slate-900 border-purple-500 shadow-xl shadow-purple-500/20' : 'bg-surface border-slate-700 hover:border-purple-500/50'}`}>
          ...
          {!user.isPremium ? (
             <div className="space-y-3">
               {isNative ? (
                 <Button onClick={handleUpgrade} className="w-full bg-green-700 hover:bg-green-600">
                    Subscribe with Google Play
                 </Button>
               ) : (
                 <GooglePayButton onClick={handleUpgrade} />
               )}
               <p className="text-center text-[10px] text-slate-500">
                 {isNative ? "Secure transaction via Google Play Store" : "Secure transaction via Stripe / Google Pay"}
               </p>
             </div>
          ) : (
